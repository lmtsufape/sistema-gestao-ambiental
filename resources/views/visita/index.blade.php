        ...
        <div class="form-row justify-content-center">
            <div class="col-md-9">
                <ul class="nav nav-tabs nav-tab-custom" id="myTab" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link @if($filtro == 'denuncia') active @endif" id="visitas-finalizados-tab" role="tab" type="button" @if($filtro=='denuncia' ) aria-selected="true" @endif href="{{route('visitas.index', ['filtro' => 'denuncia', 'ordenacao' => 'data_marcada', 'ordem' => 'DESC'])}}">Denúncias</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link @if($filtro == 'finalizado') active @endif" id="visitas-finalizadas-tab" role="tab" type="button" @if($filtro=='finalizado' ) aria-selected="true" @endif href="{{route('visitas.index', ['filtro' => 'finalizado', 'ordenacao' => 'data_marcada', 'ordem' => 'DESC'])}}">Finalizados</a>
                    </li>
                    @endcan
                    @can('isAnalistaPodaOrSecretario', \App\Models\User::class)
                    <li class="nav-item">
                        <a class="nav-link @if($filtro == 'poda') active @endif" id="visitas-cancelados-tab" role="tab" type="button" @if($filtro=='poda' ) aria-selected="true" @endif href="{{route('visitas.index', ['filtro' => 'poda', 'ordenacao' => 'data_marcada', 'ordem' => 'DESC'])}}">Poda/Supressão</a>
                    </li>
                    @endcan
                </ul>
                            ...
                            <table class="table" id="programacao-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @elseif(isset($visita->requerimento) && isset($visita->requerimento->empresa))
                                        @if($visita->requerimento->notificacoes && $visita->requerimento->notificacoes->where('empresa_id', $visita->requerimento->empresa->id)->isNotEmpty())
                                        <td>{{__('Notificado')}}</td>
                                        @endif
                                        @else
                                        <td>{{__('Aguardando visita')}}</td>
                                        ...
                                        <td>
                                            ...
                                            <a title="Relatório" href="{{route('relatorios.show', ['relatorio' => $visita->relatorio])}}">
                                                <img class="icon-licenciamento" @if ($visita->relatorio->aprovacao == \App\Models\Relatorio::APROVACAO_ENUM['aprovado'])
                                            ...
                                            <a title="Relatório" href="@if($visita->relatorio != null){{route('relatorios.edit', ['relatorio' => $visita->relatorio])}}@else{{route('relatorios.create', ['visita' => $visita->id])}}@endif"><img class="icon-licenciamento" @if ($visita->relatorio != null && $visita->relatorio->aprovacao == \App\Models\Relatorio::APROVACAO_ENUM['aprovado'])
                                                src="{{asset('img/Relatório Aprovado.svg')}}"
                                                @else
                                                src="{{asset('img/Relatório Sinalizado.svg')}}"
                                                ...
                                                <a title="Notificações" href="{{route('empresas.notificacoes.index', ['empresa' => $visita->requerimento->empresa])}}"><img class="icon-licenciamento" src="{{asset('img/notification-svgrepo-com.svg')}}" alt="Icone de notificações"></a>
                                                @endif
                                                @if($visita->requerimento->tipo == \App\Models\Requerimento::TIPO_ENUM['renovacao'])
                                                <a title="Relatórios Antigos" href="{{route('recuperar.relatorios', ['requerimento' => $visita->requerimento])}}">
                                                    <img class="icon-licenciamento" width="20" src="{{ asset('img/relatorios antigos.svg') }}" alt="Icone de relatórios antigos">
                                                    @endif
                                                    @endif
                                                    @elseif ($visita->denuncia != null)
            ...
            <div class="col-md-3" style="margin-top: 2.5rem;">
                <div class="col-md-12 shadow-sm p-2 px-3" style="background-color: #ffffff; border-radius: 00.5rem;">
                    ...
                    <ul class="list-group list-unstyled mt-2">
                        ...
                        <li>
                            <div title="Visualizar relatório" class="d-flex align-items-center my-1 pt-0 pb-1">
                                <img class="icon-licenciamento aling-middle" width="20" src="{{ asset('img/relatorios antigos.svg') }}" alt="Visualizar relatórios antigos">
                                <div style="font-size: 15px;" class="aling-middle mx-3">
                                    Relatórios antigos
                                </div>
                            </div>
